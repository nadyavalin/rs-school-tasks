...
export const carArea = createDiv("car-area");
...
export const modelText = createText("model-text", "");
...
function createCarName() {
  ...
  modelText.textContent = inputCarName;
  if (!carAreaButtons.contains(modelText)) {
    carAreaButtons.append(modelText);
  }
}
